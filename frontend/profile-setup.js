...
// Manages which entry in a list is open for editing.
// Automatically saves the current data when the user switches between entries.
// Use startEdit for clicks on existing entries, setEditingId for new entries
// (adding a new entry doesn't need to trigger an auto-save of the prior state).
function useEditingId(data, onSave) {
  ...
  // Clears editingId only if the given id is the one currently open.
  // Used by remove() so deleting a non-open entry doesn't close the open one.
  const clearEdit = (id) => {
    if (editingId === id) setEditingId(null);
  };

  return { editingId, setEditingId, startEdit, finishEdit, clearEdit };
}

// ── Helpers ───────────────────────────────────────────────────────────────────

// Shared bullet CRUD for any list whose items contain bullet_points.
...
//   data     — the section's array (or string for ProfileSection)
//   onChange — updates that slice in parent state
...
// data = profile string (not the full candidate object)
...
function SkillsSection({ data, onChange, onSave }) {
  ...
  const add = () => {
    const newId = crypto.randomUUID();
    onChange([...data, { _id: newId, name: "", skills: [] }]);
    setEditingId(newId);
  };
...
function CertSection({ data, onChange, onSave }) {
  ...
  const add = () => {
    const newId = crypto.randomUUID();
    onChange([...data, { _id: newId, name: "", issuer: "", date: "" }]);
    setEditingId(newId);
  };
...
function SummarySection({ data, onChange, onSave }) {
  ...
  const add = () => {
    const newId = crypto.randomUUID();
    onChange([...data, { _id: newId, topic: "", response: "" }]);
    setEditingId(newId);
  };
...
function ResumeSettingsSection({ data, onChange }) {
  ...
  async function exportResume() {
    ...
    try {
      ...
      const res = await fetch("/api/v1/resume/download/" + result.pdf_url);
...
function UploadResumeSection({ hasExistingProfile, onUploadComplete }) {
...
function ProgressBar({ lines, minLines }) {
  ...
  const baseColor = TIER_COLORS[Math.min(tier, 3)];
  const nextColor = TIER_COLORS[Math.min(tier + 1, 3)];
  const fillWidth = (tier === 0 ? Math.min(pct, 100) : pct % 100) + "%";
...
function App() {
  ...
  // Snapshot ref gives commitCurrent access to current state without stale closures.
  const snap = useRef({ personal, candidate, summary, layout, tab, loading });
  useEffect(() => {
  ...
  // data = { topics: [...] } — the full summary wrapper object
  ...
  async function saveLayoutData(data) {
    await apiFetch("PUT", "/api/v1/profile/layout", { section_order: data });
  }
  ...
  async function init() {
    ...
    try {
      ...
      if (data?.personal) setPersonal({ ...BLANK_PERSONAL, ...data.personal });
    ...
    try {
      ...
      console.warn("Failed to load layout — using default:", e.message);
    }
    setLoading(false);
    refreshLineCount();
  }
  ...
  // ── Upload callback (defined here, not inside renderSection switch) ──────────
  ...
  // ── Section rendering ────────────────────────────────────────────────────────
  // Each section receives only its own slice of candidate state.
  // makeCandSave / makeCandChange wire that slice back into the full candidate
  ...
  function renderSection() {
    ...
    switch (tab) {
      ...
      case "resume_settings":
        return <ResumeSettingsSection data={layout} onChange={setLayout} />;
    }
  }
